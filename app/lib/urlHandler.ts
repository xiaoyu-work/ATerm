...
export function isAtermURL (arg: string): boolean {
    return arg.toLowerCase().startsWith('aterm://')
}

export function parseAtermURL (url: string, cwd: string = process.cwd()): any {
    try {
        if (!isAtermURL(url)) {
            return null
        }
        ...
        if (!commandConfig) {
            console.error(`Unknown command in aterm:// URL: ${command}`)
            return null
        }
        ...
        console.error('Failed to parse aterm:// URL:', e)
        return null
    }
}
